...
private:
  ...
  bool userAdded = false; ///< True if the resource was added by the user, and
                          ///< not automatically by GDevelop.
...
public:
  ImageResource() : Resource(), smooth(true) { SetKind("image"); };
...
public:
  AudioResource()
      : Resource(), preloadAsMusic(false), preloadAsSound(false),
  ...
  /**
   * \brief Return true if the audio resource should be preloaded in cache
   * (without decoding into memory).
   */
  bool PreloadInCache() const { return preloadInCache; }

  /**
   * \brief Set if the audio resource should be preloaded in cache (without
 ...
 * \ingroup ResourcesManagement
 */
class GD_CORE_API ResourcesContainer {
public:
  enum SourceType {
    Unknown,
    Global,
   ...
   */
  const gd::String &
  GetResourceNameWithOrigin(const gd::String &originName,
   ...
   */
  std::vector<gd::String> FindFilesNotInResources(
   ...
   */
  std::size_t Count() const { return resources.size(); };

  /**
   * \brief Clear all variables of the container.
   */
  inline void Clear() { resources.clear(); }
   ...
   */
  static void SerializeResourceTo(gd::Resource &resource,
