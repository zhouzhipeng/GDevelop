...
describe('add_scene_events', () => {
  ...
  it('applies direct serialized events without calling the generation service', async () => {
    // $FlowFixMe[underconstrained-implicit-instantiation]
    const generateEvents = jest.fn();
    // $FlowFixMe[underconstrained-implicit-instantiation]
    ...
    const result: EditorFunctionGenericOutput = await editorFunctions.add_scene_events.launchFunction(
      {
        ...
        args: {
          scene_name: 'TestScene',
          generated_event_id: 'direct-event-id',
    ...
    expect(result.aiGeneratedEventId).toBe('direct-event-id');
    expect(result.lowLevelMutations).toBe(1);
    expect(generateEvents).not.toHaveBeenCalled();
    expect(
      project
        .getLayout('TestScene')
