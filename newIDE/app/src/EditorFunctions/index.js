...
const getEventsSourceTarget = (
  resolvedScope: ResolvedScope,
  functionName: ?string,
  lifecycleFunctionName: SceneLifecycleFunctionName = DEFAULT_SCENE_LIFECYCLE_FUNCTION_NAME
):
  | {|
      success: true,
  ...
  if (layout) {
    return {
      success: true,
      eventsList: getSceneLifecycleEvents(layout, lifecycleFunctionName),
      eventsFunction: null,
    };
  }
...
const readEventsSource: EditorFunction = {
  ...
  launchFunction: async ({ project, args, ensureExtensionsUpToDate }) => {
    ...
    const target = getEventsSourceTarget(
      resolvedScope,
      functionName,
      lifecycleFunctionName
    );
...
const addSceneEvents: EditorFunction = {
  ...
  launchFunction: async ({
    ...
    if (directEventChanges) {
      const sceneName = getSceneNameFromArgs(args);
