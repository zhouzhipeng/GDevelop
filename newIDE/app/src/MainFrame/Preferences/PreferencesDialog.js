...
const sections: Array<PreferencesSection> = [
  {
    ...
    getIcon: ({ color, fontSize }) => (
      <SettingsIcon color={color} fontSize={fontSize} />
    ),
  },
  {
    name: 'general',
  ...
  const getSectionContent = (
    ...
    switch (sectionName) {
      case 'mcp-server':
        return {
          settings: [],
          renderHeader: electron ? () => mcpServerPreferences : undefined,
        };
      case 'general':
        return {
          settings: [
      ...
      case 'ask-ai':
        return {
          settings: [
            {
              id: 'show-add-note-button-in-title-bar',
              label: i18n._(t`Show "Add Note" button in the title bar`),
              renderControl: () =>
                renderToggle(
                  values.showAddNoteButtonInTitleBar,
                  setShowAddNoteButtonInTitleBar
                ),
            },
            {
    ...
    const matchingSections = sections
      .map(section => {
        ...
        return matchingSettings.length > 0 ||
          (isSectionNameMatching && (renderHeader || renderFooter))
