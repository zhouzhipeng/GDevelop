...
- 继续使用项目级唯一片段名称，使现有 `link external "名称"` 可以保持原样。
...
5. 片段可以引用其他片段。循环引用、缺失目标和非法 group/range 应在
   预览或导出前报告，并给出片段文件及引用链。
6. 保留现有 `group=`、历史 `range=`、禁用状态和事件元数据。
   多处引用、Trigger Once、局部变量和异步续接须保持各调用位置的隔离行为。
7. 指向场景的 Link 继续遵循场景既有生命周期规则。外部片段引用场景时，
   也传递实际调用阶段，不建立片段自己的生命周期。
8. 本次不扩展跨场景或扩展函数中的引用能力；现有支持的上下文按原地
...
2. 同一片段在四个场景阶段展开同一正文，阶段约束由调用者决定；场景 Link
   行为不变；空片段为 no-op，未引用片段不执行。
3. 嵌套 Link、循环依赖、group/range、缺失目标及调用位置诊断。
